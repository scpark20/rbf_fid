...
class RBFSolverCPDTargetLG:
    ...
    def sample_by_target_matching(self, x, target, steps, skip_type='logSNR', order=3, gamma_max=5.0, gamma_num=10):
        ...
        with torch.no_grad():
            ...
            for i in range(0, steps):
                ...
                #print('pred min loss :', min(pred_losses))
                # plt.figure(figsize=[10, 3])
                # plt.title(f'step :{i}/{steps}, p={p}')
                # plt.plot(torch.stack(pred_losses).data.cpu().numpy())
                # plt.show()
                ...
                #print('corr min loss :', min(corr_losses))    
    ...
    def sample(self, x, steps, skip_type='logSNR', order=3, gamma_max=3.0):
        ...
        with torch.no_grad():
            ...
            for i in range(0, steps):
                ...
                # ===predictor===
                beta = steps / (np.exp(gammas[0, i]) * abs(lambdas[-1] - lambdas[0]))
                x_pred = self.get_next_sample(x, i, hist, signal_rates, noise_rates, lambdas,
                                              p=p, beta=beta, corrector=False)
                ...
                # ===corrector===
                beta = steps / (np.exp(gammas[1, i]) * abs(lambdas[-1] - lambdas[0]))
                x_corr = self.get_next_sample(x, i, hist, signal_rates, noise_rates, lambdas,
                                              p=p, beta=beta, corrector=True)
                x = x_corr
        # 최종적으로 x를 반환
