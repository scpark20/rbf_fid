...
class RBFSolverNumericalSweep:
    ...
    def get_integral_vector(self, lambda_s, lambda_t, lambdas, beta):
        ...
        # lambda_s = float(lambda_s.detach().cpu())
        # lambda_t = float(lambda_t.detach().cpu())
    ...
    def sample_by_target_matching(self, x, target, steps, skip_type='logSNR', order=3, log_scale_min=-6.0, log_scale_max=6.0, log_scale_num=100, optim_lr=1e-1, optim_steps=100):
        print('log_scale_min :', log_scale_min, 'log_scale_max :', log_scale_max)
        ...
        log_scale_max_list = np.arange(log_scale_min+1.0, log_scale_max+1.0, 1.0)

        x_list = []
        recon_list = []
        optimal_log_scales_list = []
        for log_scale_max2 in log_scale_max_list:
            x = x_original
            log_scales = torch.linspace(log_scale_min, log_scale_max2, log_scale_num, device=device)
