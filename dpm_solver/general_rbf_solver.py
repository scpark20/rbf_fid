...
class GeneralRBFSolver:
    ...
    def get_kernel_function(self, name):
        if name == 'Gaussian': # Gaussian
            return lambda x, y, beta: torch.exp(-beta**2 * (x - y)**2)
        elif name == 'Laplace': # Laplace
            return lambda x, y, beta: torch.exp(-beta * torch.abs(x - y))
        elif name == 'IMQ': # Inverse Multi-Quadrics
            return lambda x, y, beta: 1 / torch.sqrt((x - y)**2 + beta**2)
        elif name == 'MUQ': # Multi-Quadrics
            return lambda x, y, beta: torch.sqrt((x - y)**2 + beta**2)
        elif name == 'NMQ': #Normalized Multi-Quadrics
            return lambda x, y, beta: torch.sqrt(1.0 + (beta * torch.abs(x - y))**2)
        else:
    ...
    def get_kernel_matrix(self, lambdas, beta):
        ...
        # (p, p)
        K = self.kernel_func(lambdas, lambdas.T, beta)
        return K
    ...
    def get_integral_vector(self, lambda_s, lambda_t, lambdas, beta):
        ...
        # (N,)
        domain = torch.linspace(lambda_s, lambda_t, self.subintervals, device=lambda_s.device)
        # (N, len(lambdas))
        if self.predict_x0:
            func_values = torch.exp(domain[:, None]) * self.kernel_func(domain[:, None], lambdas[None, :], beta)
        else:
            func_values = torch.exp(-domain[:, None]) * self.kernel_func(domain[:, None], lambdas[None, :], beta)
        # (len(lambdas),)
        integral = torch.trapz(func_values, domain, dim=0)
        return integral
    ...
    def sample_by_target_matching(self, x, target, steps, skip_type='logSNR', order=3, log_scale_max=6.0, log_scale_num=100, optim_lr=1e-1, optim_steps=100):
        ...
        if self.scale_dir is not None:
            optimal_file = os.path.join(self.scale_dir, f'NFE={steps},p={order},kernel={self.kernel_name}.npy')
            np.save(optimal_file, optimal_log_scales)
            print(optimal_file, ' saved!')

        return x, optimal_log_scales

    def sample_by_target_matching_grad(self, x, target, steps, skip_type='logSNR', order=3, log_scale_max=6.0, optim_lr=1e-1, optim_steps=100):
        # x : start noise to sample
        # target : target image to sample
        # log_scale_max : absolute value of log scale to search
        # log_scale_num : # of log scale in [-log_scale, log_scale] to search
    ...
    def load_optimal_log_scales(self, NFE, order):
        try:
            load_file = os.path.join(self.scale_dir, f'NFE={NFE},p={order},kernel={self.kernel_name}.npy')
