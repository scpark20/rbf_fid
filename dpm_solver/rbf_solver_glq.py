...
class RBFSolverGLQ:
    ...
    def get_integral_vector(self, lambda_s, lambda_t, lambdas, beta):
        ...
        # Gaussian-Legendre Quadrature 4-points
        else:    
            x = [
            ...
            w = [
                    0.3478548451374538573730639492220,
                    0.6521451548625461426269360507780,
                    0.6521451548625461426269360507780,
                    0.3478548451374538573730639492220
                ]
            
            def f1(lam):
                return torch.exp(lam - beta**2*(lam-lambdas)**2)
            def f2(lam):
                return (lambda_t-lambda_s)/2 * f1(lam*(lambda_t-lambda_s)/2 + (lambda_s+lambda_t)/2)
            
            result = 0
            for i in range(len(x)):
                result = result + (w[i] * f2(x[i]))
            
            return result.float()
        
    def get_integral_vector2(self, lambda_s, lambda_t, lambdas, beta):
        ...
        # Gaussian-Legendre Quadrature 8-points
        else:    
            x = [
