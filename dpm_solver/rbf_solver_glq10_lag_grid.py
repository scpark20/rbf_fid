...
class RBFSolverGLQ10LagGrid:
    ...
    def get_loss_by_target_matching(self, i, steps, target, hist, log_scale_p, log_scale_c, lambdas, p, p_prev):
        if log_scale_c is not None:
            # for predictor
            beta = steps / (np.exp(log_scale_p) * abs(lambdas[-1] - lambdas[0]))
            ...
            # for corrector
            beta = steps / (np.exp(log_scale_c) * abs(lambdas[-1] - lambdas[0]))
    ...
    def sample_by_target_matching(self, x, target, steps, skip_type='logSNR', order=3):
        ...
        with torch.no_grad():
            ...
            for i in range(0, steps):
                ...
                if i > 0: # Grid Search
                    ...
                    for pindex, log_scale_p in enumerate(log_scales):
                        for cindex, log_scale_c in enumerate(log_scales):
                            loss = self.get_loss_by_target_matching(i, steps, target, hist, log_scale_p, log_scale_c, lambdas, p, p_prev)
                    ...
                    for pindex, log_scale_p in enumerate(log_scales):
                        loss = self.get_loss_by_target_matching(i, steps, target, hist, log_scale_p, None, lambdas, p, p_prev)
                ...
                if i > 0:
                    ...
                    beta = steps / (np.exp(log_s) * abs(lambdas[-1] - lambdas[0]))
                    x = self.get_next_sample(x, i-1, hist, signal_rates, noise_rates, lambdas,
                                             p=p_prev, beta=beta, corrector=True, lagrange=(log_s >= self.log_scale_max))
                
                # ===Predictor===
                log_s = optimal_log_scales[0, i]
                beta = steps / (np.exp(log_s) * abs(lambdas[-1] - lambdas[0]))
                x_pred = self.get_next_sample(x, i, hist, signal_rates, noise_rates, lambdas,
                                              p=p, beta=beta, corrector=False, lagrange=(log_s >= self.log_scale_max))
                p_prev = p
            x = x_pred   
        ...
        # 최종적으로 x를 반환
        return x, optimal_log_scales, loss_grid_list
    ...
    def sample_ecp(self, x, steps, skip_type='logSNR', order=3, log_scale=0.0):
        ...
        with torch.no_grad():
            ...
            for i in range(0, steps):
                ...
                if i > 0:
                    # ===Corrector===
                    s = log_scale if log_scales is None else log_scales[1, i-1]
                    beta = steps / (np.exp(s) * abs(lambdas[-1] - lambdas[0]))
                    print('Corr x', i-1, p_prev)
                    x = self.get_next_sample(x, i-1, hist, signal_rates, noise_rates, lambdas,
                                             p=p_prev, beta=beta, corrector=True, lagrange=(s >= self.log_scale_max))
                ...
                print('Pred x', i, p)
                x_pred = self.get_next_sample(x, i, hist, signal_rates, noise_rates, lambdas,
                                              p=p, beta=beta, corrector=False, lagrange=(s >= self.log_scale_max))
                p_prev = p
            x = x_pred        
        return x, hist       