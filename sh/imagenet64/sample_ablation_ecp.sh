...
for steps in 15 20 25 30 35 40
do
    sampleMethod='rbf_ecp_marginal'
...
for steps in 40
do
    sampleMethod='rbf_ecp_marginal_lagp'
    workdir="samples/64x64_diffusion/${sampleMethod}_order3_${steps}"
    echo "===== Running with method=${sampleMethod}, order=3, steps=${steps} ====="
    CUDA_VISIBLE_DEVICES="${DEVICES}" python main.py \
        --config "${data}.yml" \
    ...
    sampleMethod='rbf_ecp_marginal_lagc'
    workdir="samples/64x64_diffusion/${sampleMethod}_order3_${steps}"
    echo "===== Running with method=${sampleMethod}, order=3, steps=${steps} ====="
    CUDA_VISIBLE_DEVICES="${DEVICES}" python main.py \
        --config "${data}.yml" \
        ...
        --fid \
        --timesteps "${steps}" \
        --scale_dir "/data/guided-diffusion/scale/rbf_ecp_marginal_lagc_64" \
        --eta 0 \
        --ni \
        --skip_type "${DIS}" \
        --sample_type "${sampleMethod}" \
    ...
    sampleMethod='rbf_ecp_marginal_spd'
    workdir="samples/64x64_diffusion/${sampleMethod}_order3_${steps}"
    echo "===== Running with method=${sampleMethod}, order=3, steps=${steps} ====="
    CUDA_VISIBLE_DEVICES="${DEVICES}" python main.py \
        --config "${data}.yml" \
        ...
        --fid \
        --timesteps "${steps}" \
        --scale_dir "/data/guided-diffusion/scale/rbf_ecp_marginal_spd_64" \
        --eta 0 \
        --ni \
        --skip_type "${DIS}" \
        --sample_type "${sampleMethod}" \
    ...
    sampleMethod='rbf_ecp_marginal_to1'
    workdir="samples/64x64_diffusion/${sampleMethod}_order3_${steps}"
    echo "===== Running with method=${sampleMethod}, order=3, steps=${steps} ====="
    CUDA_VISIBLE_DEVICES="${DEVICES}" python main.py \
        --config "${data}.yml" \
    ...
    sampleMethod='rbf_ecp_marginal_to3'
    workdir="samples/64x64_diffusion/${sampleMethod}_order3_${steps}"
    echo "===== Running with method=${sampleMethod}, order=3, steps=${steps} ====="
    CUDA_VISIBLE_DEVICES="${DEVICES}" python main.py \
        --config "${data}.yml" \
        ...
        --fid \
        --timesteps "${steps}" \
        --scale_dir "/data/guided-diffusion/scale/rbf_ecp_marginal_to3_64" \
        --eta 0 \
        --ni \
        --skip_type "${DIS}" \
        --sample_type "${sampleMethod}" \
